...
export default async function VodReportPage({
  ...
  return (
    <>
      ...
      {/* Back to summary */}
      <div>
        <Link href={`/dashboard/vods/${id}`} className="btn btn-ghost" style={{ padding: "6px 12px", fontSize: 12, marginBottom: 4 }}>
          <Icons.Back /> Back to summary
        </Link>
      </div>
      ...
      {(readyClips.length > 0 || processingClips.length > 0 || failedClips.length > 0) && (
        <div style={{ marginTop: 32 }}>
          <div style={{ marginBottom: 14, display: "flex", alignItems: "center", gap: 14 }}>
            <span className="page-eyebrow">Generated Clips ({readyClips.length})</span>
