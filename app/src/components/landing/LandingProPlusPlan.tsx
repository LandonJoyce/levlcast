...
export default function LandingProPlusPlan() {
  const [annual, setAnnual] = useState(false);

  // Annual = $299/yr (≈$24.92/mo, saves $60.88 vs monthly $359.88)
  // Monthly headline rate: $29.99
  ...
  return (
    <article className="ll-plan ll-plan-plus">
      ...
      <div>
        ...
        <div className="ll-plan-cycle">{annual ? "billed yearly" : "For heavy streamers"}</div>
      </div>

      <div className="ll-plan-price" style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        {annual ? (
          <>
            <span>$299 <small>/ year</small></span>
            <span style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", textDecoration: "line-through", fontWeight: 400 }}>$359.88</span>
          </>
        ) : (
          <span>$29.99 <small>/ month</small></span>
        )}
      </div>
      ...
      <Link
        href={annual ? "/auth/login?plan=pro_plus_annual" : "/auth/login?plan=pro_plus"}
        className="ll-btn ll-btn-grad ll-btn-arrow"
      >
        {annual ? "Get Pro Plus — $299/year" : "Get Pro Plus"}
      ...
      {!annual && (
        ...
        >
          Switch to annual and save $60.88
        </button>
      )}
    </article>
  );
